...
def build_parser() -> argparse.ArgumentParser:
    ...
    enroll_p.add_argument(
        "--re-enroll", action="store_true", dest="re_enroll",
    ...
    enroll_p.add_argument(
        "--model", choices=["buffalo_sc", "buffalo_l"], default=None,
        help="InsightFace model (default: buffalo_sc — fast CPU; "
             "buffalo_l — higher accuracy, ~500 MB download on first use)",
    )

    # ── recognize ───────────────────────────────────────────────────────
    rec_p = sub.add_parser("recognize", help="Run real-time face recognition")
    rec_p.add_argument(
        "--camera", type=int, default=None,
    ...
    # ── liveness ─────────────────────────────────────────────────────────
    live_p = sub.add_parser("liveness", help="Run a standalone liveness challenge")
    live_p.add_argument(
        "--camera", type=int, default=None,
        help="Camera device index (default: 0)",
    )
    ...
    del_p.add_argument("--user", required=True, help="Username to remove")
...
def main() -> int:
    ...
    log.info("Ubuntu FaceAuth started — command=%s", args.command)
    ...
    if args.command == "enroll":
        ...
        session = EnrollmentSession(settings, detector, store)
    ...
    if args.command == "recognize":
        ...
        if not enrolled:
            ...
            log.warning("Recognition started with no enrolled users")

        runner = RecognitionRunner(settings, detector, store)
        runner.run(debug=getattr(args, "debug", False),
    ...
    if args.command == "authenticate":
        ...
        if headless:
            session = HeadlessAuthSession(settings, detector, store)
        else:
            session = AuthSession(settings, detector, store)
        ...
        log.info(
            ...
            args.user, result.outcome.name, result.similarity,
        )
